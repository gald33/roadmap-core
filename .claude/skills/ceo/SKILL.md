...
```bash
org tempo --repo /home/user/roadmap-core register --role ceo     # your agent id into coord/tempo/agents, so senders can wake you; answers `listener_alive` for step 6
...
```bash
org operator --repo /home/user/roadmap-core render     # every role's three lines from roles/<role>/last-run; then commit the file on main
```
...
field, then `org tempo --repo /home/user/roadmap-core dm --role ceo "blocked_written ceo"` —
