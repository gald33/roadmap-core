...
```bash
org tempo --repo /home/user/roadmap-core register --role checks-watch     # your agent id into coord/tempo/agents, so senders can wake you; answers `listener_alive` for step 6
...
```bash
export SUITE="$(org setup --repo /home/user/roadmap-core status --compact)"     # the org-core and suite versions this container runs (doctor check 21)
switchboard --json board set roles/checks-watch/last-run "$(python3 -c '
import json, datetime, os
print(json.dumps({
