...
```bash
org tempo --repo /home/user/roadmap-core register --role doctor     # your agent id into coord/tempo/agents, so senders can wake you; answers `listener_alive` for step 6
...
`/home/user/roadmap-core` is a shared checkout: any session may move its branch under
...
```bash
cd /home/user/roadmap-core && git fetch origin main --quiet
...
Never `git checkout -B` in `/home/user/roadmap-core` itself: it moves the branch under
...
gh api repos/gald33/roadmap-core/pulls -f title="doctor $(date -u +%Y-%m-%d)" -f head="$(git branch --show-current)" -f base=main \
