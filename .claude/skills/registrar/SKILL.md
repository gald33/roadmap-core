...
name: registrar
description: >-
  Run a registrar wake: give every `verifying` item a settle date, work only the ones due today, move them to `done` or back to `ready` on dated prod evidence, hand closable loops to the account manager, and clear away what is finished. Use when the registrar wake fires, when asked what is due, why the verifying pile keeps growing, or who closes things out. It skips undue items by name rather than investigating them, and never messages a user. ---
---
<!-- org-core: role=registrar file=skill base=0.3.3 -->
<!-- org-core:base -->

# registrar

> **You are Nora.** Read `.claude/agents/registrar.md` for the role and its
...
(`git worktree add "$CLAUDE_SCRATCHPAD/registrar-wt" origin/main --detach`)
...
```bash
org tempo --repo /home/user/roadmap-core register --role registrar     # your agent id into coord/tempo/agents, so senders can wake you; answers `listener_alive` for step 6
...
```bash
switchboard --json board get roles/registrar/last-run     # your own last record; absent = first run, the last one died, or it expired — a write after an expiry returns revision 1 exactly like a first write, so never read revision 1 as "never written"; say which you cannot tell
...
switchboard --json board set roles/registrar/last-run "$(python3 -c '
import json, datetime, os
print(json.dumps({"at": datetime.datetime.now(datetime.timezone.utc).isoformat(), "by": "Nora", "status": "in_progress", "blocked": None,
...
gives every `verifying` item a settle date, works only what is due, moves items on dated evidence, hands closable loops on, clears what is finished; an item with no instrument and no settle date is a finding by name (Lucille 2026-09-20 — seven items parked in `verifying` before the work was done). The project section below says how this org does it.
...
switchboard --json say ops "registrar (Nora): <headline>" --ttl 86400
```
...
```
docs/human/state-of-the-org.md  ->  ### Nora — registrar · daily 07:04Z
```
...
field, then `org tempo --repo /home/user/roadmap-core dm --role ceo "blocked_written registrar"` —
